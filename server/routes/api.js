...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "routes/uploads/user-profile-images");
  },
...
function deleteImage(currentImage) {
  if (currentImage) {
    fs.unlink("routes/uploads/user-profile-images/" + currentImage, (err) => {
      if (err) {
        console.error(err);
        return;
      }
        ...
        conn.query(
          ...
          function (err, rows, fields) {
            ...
            if (rows.length >= 1 && rows[0].active === 1) {
              conn.query(
                "SELECT * from licence_per_user l where l.superadmin_id = ? and (l.expiration_date >= ? or l.licence_id = 1)",
...
router.post("/sendSMS", function (req, res) {
  ...
  request(
    { rejectUnauthorized: false, url: link + "getAvailableAreaCode" },
    function (error, response, codes) {
      ...
      if (checkAvailableCode(phoneNumber, JSON.parse(codes))) {
        ...
        request(
          {
            rejectUnauthorized: false,
            url: link + "/getTranslationByCountryCode/" + req.body.countryCode,
          },
          function (error, response, body) {
                  ...
                  if (rows) {
                    rows.forEach(async function (to, i, array) {
                      ...
                      if (
                        ...
                        if (language.smsSignatureWebsite) {
                          signature += " \n" + language.smsSignatureWebsite;
                        }
    ...
    connection.getConnection(function (err, conn) {
      ...
      if (req.body.mode && req.body.mode === "mail") {
        var checkAdditionalQuery =
          "(c.email != '' and c.email IS NOT NULL) and c.sendMassiveEmail = 1 ";
...
router.post("/sendRequestForDemoAccount", function (req, res, next) {
  var body = JSON.parse(
    fs.readFileSync("./routes/dynamic-mail-server/config.json", "utf-8")
  );
...
router.post("/sendReqestForDemoAccountFull", function (req, res, next) {
  ...
  // this need to make real back status response
  res.json(true);
});
...
router.post(
  "/uploadeEmployeeProfileImage/:id",
  ...
  (req, res) => {
    ...
    const data = readImageFile(
      "routes/uploads/user-profile-images/" + req.file.filename
    );
...
router.post("/createClinic", function (req, res, next) {
  connection.getConnection(function (err, conn) {
    ...
    if (!req.body.password) {
      ...
      createPassword = sha1(req.body.password);
    }
...
router.get("/getDrafts/:id/:type", function (req, res, next) {
  connection.getConnection(function (err, conn) {
    ...
    var id = req.params.id;
    conn.query(
      "select * from marketing_drafts where superadmin = ? AND type = '" +
        req.params.type +
        "'",
      [id],
      function (err, rows) {
...
router.get("/getLicenceForUser/:superadmin_id", function (req, res, next) {
  connection.getConnection(function (err, conn) {
    if (err) {
      logger.log("error", err.sql + ". " + err.sqlMessage);
      res.json(err);
...
router.post("/updateLicence", function (req, res, next) {
  connection.getConnection(function (err, conn) {
    ...
    conn.query(
      "update licence_per_user set ? where id = ?",
      [date, req.body.licenceId],
      function (err, rows, fields) {
        console.log(err);
        if (err) {
          conn.release();
...
router.post("/updateStripeAccountId", function (req, res, next) {
  connection.getConnection(function (err, conn) {
    if (err) {
      logger.log("error", err.sql + ". " + err.sqlMessage);
      res.json(err);
    ...
    conn.query(
      ...
      [req.body.stripeId, Number(req.body.superadminId)],
      function (err, rows, fields) {
        conn.release();
        if (err) {
          logger.log("error", err.sql + ". " + err.sqlMessage);
...
router.get("/checkStripeAccount/:superadminId", (req, res, next) => {
  try {
    connection.getConnection(function (err, conn) {
      if (err) {
        console.error("SQL Connection error: ", err);
