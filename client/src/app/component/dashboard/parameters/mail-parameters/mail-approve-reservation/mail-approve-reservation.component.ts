...
@Component({
  selector: "app-mail-approve-reservation",
  templateUrl: "./mail-approve-reservation.component.html",
  styleUrls: ["./mail-approve-reservation.component.scss"],
})
export class MailApproveReservationComponent implements OnInit, FormGuardData {
  @ViewChild(DynamicFormsComponent) form: DynamicFormsComponent;
  public path = "parameters";
  public file = "mail-approve-reservation";
  ...
  getData(id) {
    this.service.getMailApproveReservation(id).subscribe((data) => {
      this.data = data;
  ...
  receiveConfirm(event) {
    if (event) {
      ...
      if (this.data && this.data.length) {
        this.service
          .updateMailApproveReservation(this.changeData)
          .subscribe((data) => {
            if (data) {
        ...
        this.service
          .createMailApproveReservation(this.changeData)
          .subscribe((data) => {
            if (data) {
              this.helpService.successToastr(
                this.language.successExecutedActionTitle,
