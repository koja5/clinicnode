...
@Component({
  selector: 'app-mail-patient-form-registration',
  templateUrl: './mail-patient-form-registration.component.html',
  styleUrls: ['./mail-patient-form-registration.component.scss']
})
export class MailPatientFormRegistrationComponent implements OnInit, FormGuardData {

  @ViewChild(DynamicFormsComponent) form: DynamicFormsComponent;
  public path = "parameters";
  public file = "mail-patient-form-registration";
  ...
  getData(id) {
    this.service.getMailPatientFormRegistration(id).subscribe((data) => {
      this.data = data;
  ...
  receiveConfirm(event) {
    if (event) {
      ...
      if (this.data && this.data.length) {
        this.service
          .updateMailPatientFormRegistration(this.changeData)
          .subscribe((data) => {
            if (data) {
        ...
        this.service
          .createMailPatientFormRegistration(this.changeData)
          .subscribe((data) => {
            if (data) {
              this.helpService.successToastr(
                this.language.successExecutedActionTitle,
