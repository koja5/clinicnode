...
@Component({
  selector: 'app-mail-patient-created-account',
  templateUrl: './mail-patient-created-account.component.html',
  styleUrls: ['./mail-patient-created-account.component.scss']
})
export class MailPatientCreatedAccountComponent implements OnInit, FormGuardData {

  @ViewChild(DynamicFormsComponent) form: DynamicFormsComponent;
  public path = "parameters";
  public file = "mail-patient-created-account";
  ...
  getData(id) {
    this.service.getMailPatientCreatedAccount(id).subscribe((data) => {
      this.data = data;
  ...
  receiveConfirm(event) {
    if (event) {
      ...
      if (this.data && this.data.length) {
        this.service
          .updateMailPatientCreatedAccount(this.changeData)
          .subscribe((data) => {
            if (data) {
        ...
        this.service
          .createMailPatientCreatedAccount(this.changeData)
          .subscribe((data) => {
            if (data) {
              this.helpService.successToastr(
                this.language.successExecutedActionTitle,
