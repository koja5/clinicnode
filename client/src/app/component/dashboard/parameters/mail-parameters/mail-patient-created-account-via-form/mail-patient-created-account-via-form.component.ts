...
@Component({
  selector: "app-mail-patient-created-account-via-form",
  templateUrl: "./mail-patient-created-account-via-form.component.html",
  styleUrls: ["./mail-patient-created-account-via-form.component.scss"],
})
export class MailPatientCreatedAccountViaFormComponent implements OnInit, FormGuardData {
  @ViewChild(DynamicFormsComponent) form: DynamicFormsComponent;
  public path = "parameters";
  public file = "mail-patient-created-account-via-form";
  ...
  getData(id) {
    this.service.getMailPatientCreatedAccountViaForm(id).subscribe((data) => {
      this.data = data;
  ...
  receiveConfirm(event) {
    if (event) {
      ...
      if (this.data && this.data.length) {
        this.service
          .updateMailPatientCreatedAccountViaForm(this.changeData)
          .subscribe((data) => {
            if (data) {
        ...
        this.service
          .createMailPatientCreatedAccountViaForm(this.changeData)
          .subscribe((data) => {
            if (data) {
              this.helpService.successToastr(
                this.language.successExecutedActionTitle,
