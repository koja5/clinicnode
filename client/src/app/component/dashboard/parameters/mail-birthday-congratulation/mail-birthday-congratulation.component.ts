...
@Component({
  selector: "app-mail-birthday-congratulation",
  templateUrl: "./mail-birthday-congratulation.component.html",
  styleUrls: ["./mail-birthday-congratulation.component.scss"],
})
export class MailBirthdayCongratulationComponent
  implements OnInit, FormGuardData
{
  @ViewChild(DynamicFormsComponent) form: DynamicFormsComponent;
  public path = "parameters";
  public file = "mail-birthday-congratulation";
  public loading = true;
  public type: number;
  public id: number;
  public data: any;
  ...
  getData(id) {
    this.service.getMailBirthdayCongratulation(id).subscribe((data) => {
      this.data = data;
  ...
  receiveConfirm(event) {
    if (event) {
      ...
      if (this.data && this.data.length) {
        this.service
          .updateMailBirthdayCongratulation(this.changeData)
          .subscribe((data) => {
            if (data) {
        ...
        this.service
          .createMailBirthdayCongratulation(this.changeData)
          .subscribe((data) => {
            if (data) {
              this.helpService.successToastr(
                this.language.successExecutedActionTitle,
